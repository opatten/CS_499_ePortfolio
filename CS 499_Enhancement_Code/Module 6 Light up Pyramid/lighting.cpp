...
//CS 499 Enhancement Two Version 3
//Data Structure and Algorithm
//*Enhancements are marked below
...
#include <meshes.h>         // Mesh-related functions
...
namespace
{
	const char* const WINDOW_TITLE = ""; // Macro for window title
	...
	// Apply Textures to Shapes used, assigns it a unique identifier
	GLuint gTextureId;
	GLuint gTextureIdPyramid; //Enhancement to Pyramid 1 (top pyramid) 
	GLuint gTextureIdPyramid2; //Enhancement to Pyramid 2 (bottom pyramid) 
	GLuint gTextureIdCone;  // //Enhancement to Black Forest Trees 
...
//Uniform / Global variables for the  transform matrixes
uniform mat4 model;
uniform mat4 view;
...
void UDestroyTexture(GLuint textureId); //* Enhancement - delete the texture and release GPU resources
void UMousePositionCallback(GLFWwindow* window, double xpos, double ypos); //Mouse smovement
void UMouseScrollCallback(GLFWwindow* window, double xoffset, double yoffset); //Mouse movement
void UMouseButtonCallback(GLFWwindow* window, int button, int action, int mods); //Mouse movement
...
int main(int argc, char* argv[])
{
	...
	// Load Plane texture
	const char * texFilename = "../resources/textures/FireGround.png";//Texture for Plane
	if (!UCreateTexture(texFilename, gTextureId))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}
	// Load Pyramid 1 Texture
	texFilename = "../resources/textures/PyramidWall.png"; //Enhancement Binds Texture to Pyramid 1
	if (!UCreateTexture(texFilename, gTextureIdPyramid))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}
	// Load Pyramid 2 Texture
	texFilename = "../resources/textures/BottomPyramidWall.png"; //Enhancement to Bins Texture Pyramid 2
	if (!UCreateTexture(texFilename, gTextureIdPyramid2))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}
	// Load Black Forest Texture
	texFilename = "../resources/textures/BlackForest.png"; //Enhancement to Bind Texture to Black Forest Trees 
	...
	gCamera.Position = glm::vec3(0.0f, 3.0f, 2.0f);
	gCamera.Front = glm::vec3(1.0, -1.0, -3.0f);
...
void UProcessInput(GLFWwindow* window)
{
	...
	if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS) //* Enhancement Makes Camera View go down from current position
		gCamera.Position -= gCamera.Up * velocity;  
	if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS) //* Enhancement Makes Camera View go up from current position
		gCamera.Position += gCamera.Up * velocity;
	if (glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS) //* Enhancement Makes Camera View go up back to home position
	{
		gCamera.Position = glm::vec3(0.0f, 3.0f, 2.0f);
		gCamera.Front = glm::vec3(1.0, -1.0, -3.0f);
	...
	if (glfwGetKey(window, GLFW_KEY_O) == GLFW_PRESS)
	{
		...
		gCamera.Front = glm::vec3(0.0, 0.0, -3.0f);
		gCamera.Up = glm::vec3(0.0, 1.0, 0.0);
		gOrthographic = true;
	}
...
void URender()
{
	...
	// 1. Scales the object by 2 (Plane)
	scale = glm::scale(glm::vec3(3.0f, 3.0f, 3.0f));
	// 2. Rotates shape by 15 degrees in the x axis
	rotation = glm::rotate(0.0f, glm::vec3(2.0, 1.0f, 2.0f));
	...
	//Enhancement - Added 2nd Pyramid, add Rotation, change color, add Texture
	// 
	// 1. Scales the object by 2 (Pyramid_1)
	scale = glm::scale(glm::vec3(2.0f, 2.0f, 2.5f));
	// 2. Rotates shape by 15 degrees in the x axis
	rotation = glm::rotate(0.0f, glm::vec3(2.0, 0.0f, 0.0f)); 
	// 3. Place object at the origin
	translation = glm::translate(glm::vec3(-0.3f, 2.7f, -1.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	...
	glUniform3f(ambColLoc, 0.1f, 0.1f, 0.1f);
	glUniform3f(light1ColLoc, 1.0f, 1.0f, 1.0f);
	glUniform3f(light1PosLoc, 1.0f, 2.0f, -4.0f);
	glUniform3f(light2ColLoc, 1.0f, 1.0f, 1.0f);
	glUniform3f(light2PosLoc, 0.0f, 2.0f, 6.0f);
	//set specular intensity
	glUniform1f(specInt1Loc, 1.0f);
	...
	// 1. Scales the object by 2 (Pyramid_2)
	scale = glm::scale(glm::vec3(1.9f, 1.0, 1.9f));
	// 2. Rotates shape by 15 degrees in the x axis
	rotation = glm::rotate(3.15f, glm::vec3(1.0, 0.0f, 0.0f)); // Enhancement - The argument to glm::rotate is the angle of rotation in radians. 
	//In this case, it's specified as 3.15f radians, which is approximately 180 degrees. 
	// 3. Place object at the origin
	translation = glm::translate(glm::vec3(-0.3f, 1.2f, -1.10f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	...
	// bind textures on corresponding texture units
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, gTextureIdPyramid2);
	...
	// 1. Scales the object (Cone1)
	scale = glm::scale(glm::vec3(0.1f, 0.9f, 0.1f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-2.0f, 0.5f, -1.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	...
	//set ambient lighting strength
	glUniform1f(ambStrLoc, 0.8f);
	//set ambient color
	glUniform3f(ambColLoc, 0.1f, 0.1f, 0.1f);
	glUniform3f(light1ColLoc, 1.0f, 1.2f, 1.2f);
	glUniform3f(light1PosLoc, 1.0f, 1.0f, 1.0f);
	glUniform3f(light2ColLoc, 1.2f, 1.0f, 0.2f);
	glUniform3f(light2PosLoc, 1.0f, 1.0f, 1.0f);
	//set specular intensity
	glUniform1f(specInt1Loc, 1.0f);
	glUniform1f(specInt2Loc, 1.1f);
	//set specular highlight size
	glUniform1f(highlghtSz1Loc, 2.0f);
	glUniform1f(highlghtSz2Loc, 2.0f);

	// 1. Scales the object (Cone2)
	scale = glm::scale(glm::vec3(0.1f, 0.9f, 0.1f));
	// 2. Rotate the object
	...
	// 1. Scales the object (Cone4)
	scale = glm::scale(glm::vec3(0.1f, 0.9f, 0.1f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-1.9f, 0.5f, -1.5f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// 1. Scales the object (Sphere)
	scale = glm::scale(glm::vec3(0.2f, 0.2f, 0.2f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(1.0f, 2.0f, -4.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
